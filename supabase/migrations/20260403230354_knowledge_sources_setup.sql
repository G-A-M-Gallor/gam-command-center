-- Migration: 20260403230354_knowledge_sources_setup.sql
-- GAM Command Center — Knowledge Sources Setup
-- Create tables for SE S3 Knowledge Sources Sync system
...
CREATE INDEX IF NOT EXISTS idx_knowledge_sources_sync_freq ON knowledge_sources(sync_frequency) WHERE is_active = true;
...
CREATE INDEX IF NOT EXISTS idx_knowledge_sync_history_synced_at ON knowledge_sync_history(synced_at DESC);
...
-- ── SEED DATA: 18 KNOWLEDGE SOURCES ────────────────────────────────────
...
-- Insert 18 knowledge sources (as per briefing)
...
('System Index', 'notion', NULL, 'system-index-page-id', '{"type": "system_index"}', 'daily', true),

-- Design system sources (3 sources)
('Design Token System', 'design_system', NULL, 'design-tokens-page-id', '{"type": "design_tokens"}', 'daily', true),
('Visual Language Schema', 'design_system', NULL, 'visual-language-page-id', '{"type": "visual_language"}', 'daily', true),
('UI Component Registry', 'design_system', NULL, 'ui-components-page-id', '{"type": "ui_components"}', 'daily', true),
