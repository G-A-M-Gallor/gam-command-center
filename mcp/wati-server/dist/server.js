...
import { z } from "zod";
// --- Config ---
const WATI_API_URL = process.env.WATI_API_URL || "https://live-mt-server.wati.io/102586";
...
// --- WATI API helpers ---
async function watiGet(path, params) {
...
// Tool: Get contacts
server.tool("get_contacts", "Get WATI WhatsApp contacts with pagination. Returns contact name, phone, status, last update, and custom params.", {
    pageSize: z
        .number()
        .min(1)
        .max(100)
    ...
    const contacts = (data.contact_list || []).map((c) => ({
        name: c.fullName || "Unknown",
        phone: c.phone,
        status: c.contactStatus,
        lastUpdated: c.lastUpdated,
        source: c.source,
        leadStage: c.customParams?.find((p) => p.name === "lead_stage")?.value,
    }));
...
server.tool("get_messages", "Get WhatsApp messages for a specific contact by phone number. Returns text messages (filters out system events).", {
    phone: z
        .string()
        .describe("WhatsApp phone number (e.g. 972501234567, without + prefix)"),
    pageSize: z
        .number()
        .min(1)
        .max(50)
    ...
    const messages = items
        .filter((m) => m.type === "text" || m.type === "image" || m.type === "document")
        .map((m) => ({
        text: m.text || "",
    ...
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify({ phone, count: messages.length, messages }, null, 2),
            },
        ],
    };
});
// Tool: Send a session message
server.tool("send_message", "Send a WhatsApp session message to a contact. Only works within 24h window of last received message.", {
    phone: z
        .string()
    ...
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify({ success: data.result === "success", response: data }, null, 2),
            },
        ],
    };
});
// Tool: Send a template message
server.tool("send_template", "Send a WhatsApp template message to a contact. Works outside 24h window.", {
    phone: z
        .string()
        .describe("WhatsApp phone number (e.g. 972501234567)"),
...
server.tool("get_contact", "Get detailed info about a specific WATI contact including all custom parameters.", {
    phone: z
        .string()
        .describe("WhatsApp phone number (e.g. 972501234567)"),
}, async ({ phone }) => {
    ...
    if (!contact) {
        return {
            content: [
                { type: "text", text: `Contact ${phone} not found` },
            ],
        };
    }
    return {
        content: [
            {
                ...
                text: JSON.stringify({
                    ...
                    leadStage: contact.customParams?.find((p) => p.name === "lead_stage")?.value,
                    customParams: Object.fromEntries((contact.customParams || []).map((p) => [p.name, p.value])),
                }, null, 2),
            },
        ],
    };
...
server.tool("get_templates", "List available WhatsApp message templates from WATI.", {}, async () => {
    const data = await watiGet("/api/v1/getMessageTemplates");
    const templates = (data.messageTemplates || []).map((t) => ({
        name: t.elementName,
        status: t.status,
        category: t.category,
        body: t.body,
