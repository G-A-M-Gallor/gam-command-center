...
server.tool("get_contacts", "Get WATI WhatsApp contacts with pagination", {
    ...
    return {
        content: [{ type: "text", text: JSON.stringify({ total: data.link?.total || 0, contacts }, null, 2) }],
    };
});
server.tool("get_messages", "Get WhatsApp messages for a contact by phone number", {
    ...
    return {
        content: [{ type: "text", text: JSON.stringify({ phone, count: messages.length, messages }, null, 2) }],
    };
});
server.tool("send_message", "Send a WhatsApp session message (within 24h window)", {
    phone: z.string().describe("Phone number, e.g. 972501234567"),
    message: z.string().describe("Message text"),
}, async ({ phone, message }) => {
    const data = await watiPost(`/api/v1/sendSessionMessage/${phone}`, { messageText: message });
    return {
        content: [{ type: "text", text: JSON.stringify({ success: data.result === "success", response: data }, null, 2) }],
    };
});
server.tool("send_template", "Send a WhatsApp template message (works outside 24h window)", {
