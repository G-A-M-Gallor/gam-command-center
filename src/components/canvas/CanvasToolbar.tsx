...
export function CanvasToolbar({
  ...
  const isRtl = layout.direction !== 'ltr';
  const toggleDirection = () => {
    setLayout({ direction: isRtl ? 'ltr' : 'rtl' });
  };
  ...
  return (
    <div className="shrink-0 border-b border-slate-700/50 bg-slate-800/95 backdrop-blur-sm">
      {/* Row 1: Navigation + Title + Save status */}
      ...
      {/* Row 2: Tools */}
      <div className="flex h-9 items-center gap-1.5 border-t border-slate-700/30 px-3">
        {/* Add fields */}
        <button
        ...
        <button
          ...
          title={isRtl ? 'RTL → LTR' : 'LTR → RTL'}
        >
          <Languages className="h-3.5 w-3.5" />
          <span className="text-[10px] font-medium">{isRtl ? 'RTL' : 'LTR'}</span>
        </button>
