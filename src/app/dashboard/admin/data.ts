...
export const routes: RouteEntry[] = [
  {
    id: 'dashboard-home', path: '/dashboard', name: 'Dashboard Home', nameHe: 'דשבורד', icon: Layers,
  ...
  },
  {
    ...
    descriptionEn: 'Accent colors, fonts, 3-layer skins (shell/content/mobile), density, brand profile, style overrides, integrations hub',
    components: [
...
export const changelogEntries: ChangelogEntry[] = [
  ...
  },
  {
    ...
    notes: 'Added aria-label to ~35 icon-only buttons and interactive elements across all 15 widget files. Every button with only an icon (no visible text) now has a descriptive aria-label in both Hebrew and English (context-appropriate). Improves screen reader experience.',
  ...
  },
  {
    ...
    notes: 'GET /api/health — 4 checks: DB connectivity (service role query), Auth service, RLS enforcement (anon should be blocked), write cycle (insert+update+delete). pgTAP tests verify RLS enabled on tables + schema columns.',
  ...
  },
  {
    ...
    feature: 'Widget Bar V2 — Display modes (normal/compact/icons-only) + universal side panel',
    ...
    route: '/dashboard',
    notes: '3 display modes (normal 48px / compact 36px / icons-only 32px) with cycle button. Bar height, icon sizes, label visibility all respond to mode. DashboardShell adjusts top padding. UniversalSidePanel opens any widget as resizable side panel (300-700px, opposite of sidebar). widgetPanelModes state in WidgetContext.',
  ...
  },
  {
    ...
    route: '/dashboard/entities/[type]',
    notes: 'Three new bulk actions (field update, status change, assign) with multi-step modal. Per-user action permissions via user_action_permissions table with role-based fallback. resolveActions filters by canPerformAction. bulkUpdateMeta enhanced with activity tracking. All 9 entity types wired with bulk actions.',
